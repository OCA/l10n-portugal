...
class stock_simplified_onshipping(osv.osv_memory):
    ...
    def view_init(self, cr, uid, fields_list, context=None):
        if context is None:
            context = {}
        res = super(stock_simplified_onshipping, self).view_init(cr, uid, fields_list, context=context)
        ...
        if len(active_ids) == 1 and count:
            raise osv.except_osv(_('Warning !'), _('This picking list does not require invoicing or already created a waybill.'))
        if len(active_ids) == count:
            raise osv.except_osv(_('Warning !'), _('None of these picking lists require invoicing or already created a waybill.'))
        return res
    ...
    def open_simplified(self, cr, uid, ids, context=None):
        ...
        action = {}
        if not invoice_ids:
            raise osv.except_osv(_('Error'), _('No Simplified Invoices were created.'))
        action_model,action_id = data_pool.get_object_reference(cr, uid, 'l10n_pt_account', "action_simplified_invoice_tree")
        if action_model:
