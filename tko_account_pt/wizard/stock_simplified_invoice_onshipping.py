...
class stock_simplified_onshipping(osv.osv_memory):
    ...
    def _get_journal_id(self, cr, uid, context=None):
        ...
        for pick in browse_picking:
            ...
            value = journal_obj.search(cr, uid, query)
    ...
    def view_init(self, cr, uid, fields_list, context=None):
        if context is None:
            context = {}
        res = super(stock_simplified_onshipping, self).view_init(
            cr, uid, fields_list, context=context)
        pick_obj = self.pool.get('stock.picking')
        count = 0
        active_ids = context.get('active_ids', [])
        for pick in pick_obj.browse(cr, uid, active_ids, context=context):
            if pick.invoice_state != '2binvoiced' or \
               pick.waybill_state != 'none':
                count += 1
        if len(active_ids) == 1 and count:
            raise osv.except_osv(
                _('Warning !'),
                _('This picking list does not require invoicing or already '
                  'created a waybill.'))
        if len(active_ids) == count:
            raise osv.except_osv(
                _('Warning !'),
                _('None of these picking lists require invoicing or already '
                  'created a waybill.'))
        return res
    ...
    def open_simplified(self, cr, uid, ids, context=None):
        ...
        if not invoice_ids:
            raise osv.except_osv(
                _('Error'),
                _('No Simplified Invoices were created.'))
