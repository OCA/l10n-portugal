...
class account_pt_config(osv.osv_memory):
    ...
    _columns = {
        ...
        'module_tko_account_pt_account_reports': fields.boolean(
            ...
            help='Overwrites the Balance, General Ledger and adds '
                 'a summary extracts report. This installs the module '
    ...
    def get_default_fields(self, cr, uid, fields, context=None):
        ...
        debit_note_query = [('code', '=', 'MISC'), ('type', '=', 'general')]
        debit_note_id = journal_obj.search(cr, uid, debit_note_query, limit=1)
        sup_query = [('code', '=', 'account.invoice.in_debit_note')]
        sup_debit_note = sequence_obj.search(cr, uid, sup_query, limit=1)
        rem_query = [('code', '=', 'account.guia.remessa.sequence')]
        waybill_rem = sequence_obj.search(cr, uid, rem_query, limit=1)
        trsp_query = [('code', '=', 'account.guia.transporte.sequence')]
        waybill_trsp = sequence_obj.search(cr, uid, trsp_query, limit=1)
        dev_query = [('code', '=', 'account.guia.devolucao.sequence')]
        waybill_dev = sequence_obj.search(cr, uid, dev_query, limit=1)
        if debit_note_id:
