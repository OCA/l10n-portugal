...
class hr_contract(osv.osv):
    ...
    _name = 'hr.contract'
    _description = _('Employee Contract')

    def write(self, cr, uid, ids, vals, context=None):
        hr_employee_obj = self.pool.get('hr.employee')
        ir_model_obj = self.pool.get('ir.model')
        model_obj_id = ir_model_obj.search(
            cr, uid, [('model', '=', 'hr.contract')])
        if context is None:
            context = {}
        for line in self.browse(cr, uid, ids):
            for value in vals:
                previous_value = self.read(cr, uid, line.id, [value])[value]
                if not any(value in d for d in self._history_vals_log):
                    continue
                index = next(index for (index, d) in enumerate(
                    self._history_vals_log) if value in d)
                object = self._history_vals_log[index][value]
                change = _(u'%s - Contract reference %s') % (
                    vals[value], line.name)
                hr_employee_obj.write_history_line(cr, uid, ids,
                                                   line.employee_id.id,
                                                   model_obj_id[0], value,
    ...
    _columns = {
        'permit_no': fields.char('Work Permit No', size=256, required=False,
                                 readonly=False),
        'visa_no': fields.char('Visa No', size=64, required=False,
                               readonly=False),
        'visa_expire': fields.date('Visa Expire Date'),
        'insurance_name': fields.char('Insurance', size=256, required=False,
                                      readonly=False),
        'insurance_no': fields.char('Insurance No', size=256, required=False,
                                    readonly=False),
...
class hr_employee(osv.osv):
    ...
    _columns = {
        'handicap': fields.boolean('Handicap',
                                   help="If is considered handicap by law"),
        'handicap_degree': fields.selection([
            ('under_60', 'Under 60%'),
            ('equal_60', 'Equal or over 60% and under 80%'),
            ('equal_80', 'Equal or over 80%')
        ], 'Handicap Degree'),
        ...
        'ssnid': fields.char('SS Identification', size=64,
                             help="Social Security Identification of"
                                  "employee"),
        'history_lines': fields.one2many('hr.employee.history', 'employee_id',
                                         'History Lines', readonly=True),
        ...
        'nif': fields.char('Tax Identification', size=64,
                           help="Tax Identification of employee"),
    }
    _defaults = {
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            for value in vals:
                previous_value = self.read(cr, uid, line.id, [value],
                ...
                # one2many relations
                if value in ('job_id'):
                    query = [('model_id', '=', model_obj_id[0]),
                             ('name', '=', value)]
                    [id] = ir_model_fields_obj.search(cr, uid, query,
                                                      context=context)
                    relation = ir_model_fields_obj.read(cr, uid, id,
                                                        ['relation'])
                    relation_name = relation['relation']
                    if vals[value]:
                        relation_obj = self.pool.get(relation_name)
                        change = relation_obj.browse(cr, uid, vals[value]).name
                    else:
                        change = 'Vazio'
                ...
                # selections
                    ...
                    if previous_value and previous_value != 'Vazio':
                        index = next(index for (index, d)
                                     in enumerate(dict)
                                     if previous_value in d)
                ...
                # text fields
                    ...
                    if vals[value]:
                        ...
                        change = 'Vazio'
                self.write_history_line(cr, uid, ids, line.id,
                                        model_obj_id[
                                            0], value, object, previous_value,
                                        change, context=context)
            super(hr_employee, self).write(cr, uid, ids, vals, context=context)
        return True

    def write_history_line(self, cr, uid, ids, employee_id, table, field,
                           object_changed, data_from, data_to, context=None):
    ...
    def onchange_ownership(self, cr, uid, ids, marital, ownership):
        ...
        if ownership and not marital:
            raise osv.except_osv(
                _('Error !'),
                _('Cannot select ownership without '
                  'the field marital selected!'))
        if ownership in ('unique', 'double') and marital != 'married':
            raise osv.except_osv(_('Error !'),
                                 _('Cannot select %s with marital: %s') % (
                self._ownership[ownership], self._marital[marital]))
        ...
        for reg in self.browse(cr, uid, ids, context=context):
            ...
            for slip in reg.slip_ids:
                if not slip.employee_id.bank_account_id.acc_number:
                    raise osv.except_osv(_('Error !'),
                                         _('Please define bank account '
                                         'for the %s employee') %
                                         (slip.employee_id.name))
                value_net = [x.amount for x in slip.line_ids if
                             # x.slip_id.state == 'done' and
                             x.salary_rule_id.category_id.code == 'NET'
                             ]
                if not value_net:
                    err_msg = _('Amount invalid for the %s employee.'
                                'Category "NET" missing in payslip.')
                    err_msg = err_msg % slip.employee_id.name
                ...
                advice_line_pool.create(cr, uid, pline, context=context)
        return True
...
class hr_payslip(osv.osv):
    ...
    _order = 'number desc'
    _columns = {
        'adhoc_rules': fields.many2many('hr.salary.rule', 'rule_adhoc_payslip',
                                        'rule_id', 'payslip_id',
                                        'Ad-hoc Rules', readonly=True,
    ...
    def cancel_sheet(self, cr, uid, ids, context=None):
        ...
        line_ids = line_pool.search(cr, uid, [('payslip_id', 'in', ids)])
    ...
    def onchange_adhoc_rules(self, cr, uid, ids, contract_id, adhoc_rules,
                             input_line_ids):
        if not contract_id:
            raise osv.except_osv(
                ("Error"), ("You must select an contract first"))
    ...
    def get_payslip_lines(self, cr, uid, contract_ids, payslip_id, lock_dict,
                          context=None):
        """
        ...
        def _sum_salary_rule_category(localdict, category, amount):
            ...
            if category.code in localdict['categories'].dict:
                value += localdict['categories'].dict[category.code]
            localdict['categories'].dict[category.code] = value
            return localdict
        ...
        class WorkedDays(BrowsableObject):
            ...
            def _sum(self, code, from_date, to_date=None):
                if to_date is None:
                    to_date = datetime.now().strftime('%Y-%m-%d')
                query = "SELECT sum(number_of_days) as number_of_days, \
                                sum(number_of_hours) as number_of_hours \
                         FROM hr_payslip as hp, hr_payslip_worked_days as pi \
                         WHERE hp.employee_id = %s AND hp.state = 'done'\
                         AND hp.date_from >= %s AND hp.date_to <= %s \
                         AND hp.id = pi.payslip_id AND pi.code = %s"
                params = (self.employee_id, from_date, to_date, code)
        ...
        class Payslips(BrowsableObject):
            ...
            def sum(self, code, from_date, to_date=None):
                if to_date is None:
                    to_date = datetime.now().strftime('%Y-%m-%d')
                query = "SELECT sum(case when hp.credit_note = False then \
        ...
        # well
        structure_ids = contract_obj.get_all_structures(cr, uid, contract_ids,
                                                        context=context)
        # get the rules of the structure and thier children
        payroll_obj = self.pool.get('hr.payroll.structure')
        rule_ids = payroll_obj.get_all_rules(cr, uid, structure_ids,
                                             context=context)

        # TKO: adding adhoc rules with permission of group_adhoc_rules
        group_id = model_data_obj.get_object_reference(cr, uid, 'tko_hr_pt',
                                                       'group_adhoc_rules')[1]
        group_obj = self.pool.get('res.groups')
        group_adhoc_rules = group_obj.browse(cr, uid, group_id,
                                             context=context)
        uids = [x.id for x in group_adhoc_rules.users]
        if group_adhoc_rules and uid in uids:
            rules_ad_hoc = self.get_extra_rules(cr, uid, payslip_id,
                                                context=context)
            rule_ids += obj_rule._recursive_search_of_rules(cr, uid,
                                                            rules_ad_hoc,
                                                            context=context)
        ...
        for contract in contract_obj.browse(cr, uid, contract_ids,
            ...
            for rule in obj_rule.browse(cr, uid, sorted_rule_ids,
                                        context=context):
                ...
                satisfies = obj_rule.satisfy_condition(cr, uid, rule.id,
                                                       localdict,
                                                       context=context)
                if satisfies and rule.id not in blacklist:
                    ...
                    if rule.code == 'SS' and not contract.no_company_ss:
                        if contract.ss_type.emp_tax_value:
                            factor1 = (contract.ss_type.emp_tax_value * -1)
                            factor2 = (contract.ss_type.com_tax_value * -1)
                            company_rate = factor1 + factor2
                        else:
                            company_rate = 0
                    elif rule.code == 'SS' and contract.no_company_ss:
                    ...
                    rule_ids = rule_obj._recursive_search_of_rules(
                        cr, uid,
                        [rule],
                        context=context)
                    blacklist += [id for id, seq in rule_ids]
    ...
    def get_worked_day_lines(self, cr, uid, contract_ids, date_from, date_to,
                             context=None):
        """
          Overwriten method to map the code of employee leave and count also
          weekends in normal working days
        """
        def was_on_leave(employee_id, datetime_day, context=None):
            holiday_obj = self.pool.get('hr.holidays')
            res = False
            day = datetime_day.strftime("%Y-%m-%d")
            query = [('state', '=', 'validate'),
                     ('employee_id', '=', employee_id),
                     ('type', '=', 'remove'),
                     ('date_from', '<=', day),
                     ('date_to', '>=', day)]
            holiday_ids = holiday_obj.search(cr, uid, query, context=context)
            if holiday_ids:
                # TKO: Map leave code
                first_holiday = holiday_obj.browse(cr, uid, holiday_ids,
                                                   context=context)[0]
                res = {
                    'leave_type': first_holiday.holiday_status_id.name,
                    'leave_code': first_holiday.holiday_status_id.code,
                }
            return res
        ...
        for contract in contract_obj.browse(cr, uid, contract_ids,
            ...
            for day in range(0, nb_of_days):
                work_hours = contract.working_hours
                end_date = day_from + timedelta(days=day)
                working_hours_on_day = cal_obj.working_hours_on_day(cr,
                                                                    uid,
                                                                    work_hours,
                                                                    end_date,
                                                                    context)
                if working_hours_on_day:
                    ...
                    leave_type = was_on_leave(contract.employee_id.id,
                                              day_from + timedelta(days=day),
                                              context=context)
                    if leave_type:
                        # if he was on leave, fill the leaves dict
                        if leave_type['leave_type'] in leaves:
                            leave = leaves[leave_type['leave_type']]
                            leave['number_of_days'] += 1.0
                            leave['number_of_hours'] += working_hours_on_day
                        else:
                ...
                # TKO: Extra condition to count also weekends
                    ...
                    if leave_type:
                        if leave_type['leave_type'] in leaves:
                            leave = leaves[leave_type['leave_type']]
                            leave['number_of_days'] += 1.0
                            hours_by_days = leave['number_of_hours'] / \
                                (leave['number_of_days'] - 1)
                            leave['number_of_hours'] += hours_by_days
    ...
    def hr_verify_sheet(self, cr, uid, ids, context=None):
        """
        This method inherit verify sheet from HR to compute
        contribution register lines
        """
        ...
        for slip in self.browse(cr, uid, ids, context=context):
            for line in slip.line_ids:
                if line.salary_rule_id.register_id:
                    register = line.salary_rule_id.register_id
                    company_contrib = register_pool.compute(cr, uid,
                                                            register, slip,
                                                            slip.contract_id,
                                                            context)
                    reg_line = {
    ...
    def compute_sheet(self, cr, uid, ids, context=None):
        ...
        for payslip in self.browse(cr, uid, ids, context=context):
            ...
            old_slipline_ids = slip_line_pool.search(
                cr, uid, [('slip_id', '=', payslip.id)], context=context)
#            old_slipline_ids
            for old_line in slip_line_pool.browse(cr, uid, old_slipline_ids):
                if old_line.lock:
                    if old_line.salary_rule_id.protect_lock:
                        msg = _('A rubrica %s esta protegida não pode '
                                'alterar este valor')
                        raise osv.except_osv(_('Error !'), msg % old_line.name)

                    lock_dict[old_line.code] = old_line.amount
                slip_line_pool.unlink(cr, uid, old_line.id, context=context)
            if payslip.contract_id:
                # set the list of contract for which the rules have to be
                # applied
                contract_ids = [payslip.contract_id.id]
            else:
                # if we don't give the contract, then the rules to apply should
                # be for all current contracts of the employee
                contract_ids = self.get_contract(cr, uid, payslip.employee_id,
                                                 payslip.date_from,
                                                 payslip.date_to,
                                                 context=context)
            lines = [(0, 0, line) for line in self.get_payslip_lines(
                cr, uid, contract_ids, payslip.id, lock_dict, context=context)]
            self.write(cr, uid, [payslip.id],
                       {'line_ids': lines, 'number': number}, context=context)
        return True
...
class hr_payslip_line(osv.osv):
    ...
    def _calculate_total(self, cr, uid, ids, name, args, context):
        ...
        """
        res = super(hr_payslip_line, self)._calculate_total(
    ...
    _columns = {
        'total': fields.function(_calculate_total, method=True, type='float',
                                 string='Total',
...
class contrib_register(osv.osv):
    ...
    def _total_contrib(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for cur in self.browse(cr, uid, ids, context=context):
            current = line_pool.search(
                cr, uid, [('register_id', '=', cur.id)], context=context)
            e_month = 0.0
            c_month = 0.0
    ...
    _columns = {
        'line_ids': fields.one2many('hr.contribution.register.line',
                                    'register_id', 'Register Line',
                                    readonly=True),
        'monthly_total_by_emp': fields.function(_total_contrib, method=True,
                                                multi='dc',
                                                string='Total By Employee',
                                                digits=(16, 2)),
        'monthly_total_by_comp': fields.function(_total_contrib, method=True,
                                                 multi='dc',
    ...
    def compute(self, cr, uid, register_id, slip, contract_id, context=None):
        ...
        def _sum_salary_rule_category(localdict, category, amount):
            ...
            result = amount
            if category.code in localdict['categories'].dict:
                result += localdict['categories'].dict[category.code]
            return result
        ...
        categories_dict = {}
        categories_obj = BrowsableObject(
            self.pool, cr, uid, contract_id.employee_id.id, categories_dict)
        localdict = {
            'result': None, 'result_qty': 1.0, 'categories': categories_obj}
        ...
        for line in slip.details_by_salary_rule_category:
            ...
            localdict = _sum_salary_rule_category(
                localdict, line.salary_rule_id.category_id, line.amount)

        # check if the rule can be applied
        satisfies = obj_rule.satisfy_condition(cr, uid, register_id.rule_id.id,
                                               localdict, context=context)
        if register_id.rule_id and satisfies \
                and register_id.rule_id.id not in blacklist:
            # compute the amount of the rule
            amount, qty, rate = obj_rule.compute_rule(
                cr, uid, register_id.rule_id.id, localdict, context=context)
            # set/overwrite the amount computed for this rule in the localdict
            tot_rule = amount * qty * rate / 100.0
        else:
            # blacklist this rule and its children
            rules = obj_rule._recursive_search_of_rules(cr, uid,
                                                        [register_id.rule_id],
                                                        context=context)
            blacklist += [id for id, seq in rules]
        return tot_rule
...
class contrib_register_line(osv.osv):
    ...
    _columns = {
        ...
        'register_id': fields.many2one('hr.contribution.register', 'Register',
                                       required=False),
        ...
        'total': fields.function(_total, method=True, store=True,
...
class hr_salary_rule(osv.osv):
    ...
    _columns = {
        'register_id': fields.many2one('hr.contribution.register',
                                       ...
                                       help="Eventual third party "
                                            "involved in the salary "
...
class hr_employee_history(osv.osv):
    ...
    _columns = {
        'employee_id': fields.many2one('hr.employee', 'Employee',
                                       on_delete='cascade', required=True),
...
class payroll_advice(osv.osv):
    ...
    _columns = {
        'register_id': fields.many2one('hr.payslip.run', 'Payslip Run',
                                       required=False),
        ...
        'number': fields.char('Number', size=64, required=False,
                              readonly=True),
        'line_ids': fields.one2many('hr.payroll.advice.line', 'advice_id',
                                    'Employee Salary', required=False),
        'chaque_nos': fields.char('Chaque Nos', size=256, required=False,
                                  readonly=False),
        'company_id': fields.many2one('res.company', 'Company',
                                      required=False),
        'bank_id': fields.many2one('res.bank', 'Bank',
                                   help="Select the Bank Address from which "
                                   "the salary is going to be paid"),
    }
    _defaults = {
        'date': lambda *a: time.strftime('%Y-%m-%d'),
    ...
    def compute_sheet(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids):
            ...
            # compute new lines
            compute = datetime.strptime(line.compute_date, '%Y-%m-%d').date()
            start_date = compute.replace(day=1)
            if compute.month < 12:
                end_date = compute.replace(month=compute.month + 1, day=1)
            else:
                end_date = compute.replace(
                    year=compute.year + 1, month=1, day=1)
            start_date_str = start_date.strftime(DEFAULT_SERVER_DATE_FORMAT)
            ...
            for payslip in payslips:
                ...
                def done_and_net(line):
                    done = line.slip_id.state == 'done'
                    net = line.salary_rule_id.category_id.code in ('NET')
                    return (done and net)
                lines = payslip.line_ids
                value_net = [x.amount for x in lines if done_and_net(x)]
                if not value_net:
                    raise osv.except_osv(_('Error !'),
                                         _('Amount invalid for the %s employee'
                                         '.Category "NET" missing in payslip.')
                                         % payslip.employee_id.name)
                vals = {
                    'advice_id': line.id,
...
class payroll_advice_line(osv.osv):
    ...
    _columns = {
        'advice_id': fields.many2one('hr.payroll.advice', 'Bank Advice',
                                     required=False),
        'name': fields.char('Bank Account A/C', size=64, required=True,
                            readonly=False),
        'employee_id': fields.many2one('hr.employee', 'Employee',
                                       required=True),
    ...
    def onchange_employee_id(self, cr, uid, ids, date, employee_id):
        ...
        if employee_id:
            employee = self.pool.get(
                'hr.employee').browse(cr, uid, employee_id)
            if not employee.bank_account_id.acc_number:
                error_msg = _('Please define bank account for the %s employee')
                raise osv.except_osv(_('Error !'), error_msg % employee.name)
            else:
...
class hr_social_security_pt(osv.osv):
    ...
    account_precision = dp.get_precision('Account')
    _columns = {
        'name': fields.char('Sequence', size=512, required=False,
                            readonly=True),
        'emp_tax_value': fields.float(string='Employee Tax',
                                      digits_compute=account_precision,
                                      readonly=True),
        'com_tax_value': fields.float(string='Company Tax',
                                      digits_compute=account_precision,
                                      readonly=True),
    }
