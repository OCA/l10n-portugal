...
class AccountAssetAsset(models.Model):
    ...
    def onchange_category_id(self, cr, uid, ids, category_id, context=None):
        ...
        if category_id:
            ...
            category = category_obj.browse(cr, uid, category_id,
                                           context=context)
            res['value']['method_time'] = category.method_time
            res['value']['depreciation_rate'] = category.depreciation_rate
            res['value']['move_end_period'] = category.move_end_period
            res['value']['legal_rate_id'] = category.legal_rate_id.id
        return res
        ...
        if asset.method_time == 'percentage':
            ...
            while percentage > 0:
                if number == 0 and asset.prorata:
                    ...
                    percentage -= asset.depreciation_rate * days / total_days
                else:
                    percentage -= asset.depreciation_rate
                number += 1
            return number
        else:
    ...
    # The_constraints were redefined in order to void the parent class check_prorata
    ...
    _constraints = [
        (_check_prorata,
         'Unexpected error in the prorata constraint!', ['prorata']),
    ]
