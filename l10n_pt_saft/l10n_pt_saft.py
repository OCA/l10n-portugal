...
#    Adaptado à versão 7 por João Figueira<jjnf@communities.pt> e
#    Jorge A. Ferreira <sysop.x0@gmail.com>
#
...
class res_partner(models.Model):
    """
    Adiciona os campos requeridos pelo SAFT relativos a clientes e
    fornecedores: conservatória; numero do registo comercial; Indicadores da
    existencia de acordos de autofacturação para compras e vendas
    ...
    reg_com = fields.Char(
        string="N.Registo", size=32, help="Número do registo comercial")
    conservatoria = fields.Char(
        string="Conservatoria", size=64,
        help="Conservatória do registo comercial")
    # Adicionar campo 'indicador da existencia de acordos de 'auto-faturação'
    self_bill_sales = fields.Boolean(
        string="Sales: self-billing",
        help="Assinale se existe acordo de auto-facturação "
             "para as vendas a este parceiro")
    self_bill_purch = fields.Boolean(
        string="Purchases self-billing",
        help="Assinale se existe acordo de auto-facturação "
             "para as compras ao parceiro")
    # todo: permitir nas facturas de venda apenas parceiros clientes;
    # Não permitir desmarcar campo 'cliente' se houver vendas.


class res_company(models.Model):
    _inherit = 'res.company'
    open_journal = fields.Many2one(
        'account.journal', string="Diário de Abertura")
...
class account_tax(models.Model):
    ...
    country_region = fields.Selection(
        string="Espaço Fiscal",
        selection=[('PT', 'Continente'),
                   ('PT-AC', 'Açores'),
                   ('PT-MA', 'Madeira')],
        required=True, default="PT")
    saft_tax_type = fields.Selection(
        string="Imposto",
        selection=[('IVA', 'IVA'), ('IS', 'Imp do Selo')],
        required=True, default="IVA")
    saft_tax_code = fields.Selection(
        string="Nível de Taxa",
        selection=[('RED', 'Reduzida'),
                   ('NOR', 'Normal'),
                   ('INT', 'Intermédia'),
                   ('ISE', 'Isenta'),
                   ('OUT', 'Outra')],
    ...
    exemption_reason = fields.Char(
        string="Motivo da isenção", size=60,
        help="No caso de IVA isento, indique qual a norma do codigo do IVA "
             "que autribui a isenção")


class account_invoice(models.Model):
    """ Campos requeridos pelo saft:
    4.1.4.2. InvoiceStatus -
                ['N': Normal, 'A': Anulado, 'S':Auto-fact,
                'F': Facturado(Talão de venda)]-calculado ao exportar
    4.1.4.3. Hash - assinatura (string 200)
    4.1.4.4. HashControl  Versão da chave (string 40)
    4.1.4.8. SelfBillingIndicator - indicador de auto-facturacao
             - ler no diario relacionado com o "S" em 4.1.4.2
    4.1.4.14.13.2.  TaxCountryRegion - País ou Região do Imposto
                    -  todo: verificar novas regras da territorialidade no IVA
    4.1.4.14.13.5.  TaxAmount - Valor do imposto    - se tipo é ISelo
    4.1.4.14.14.    TaxExemptionReason - Motivo da isenção
                    - o preceito legal aplicável - ler na tabela de impostos
    """
...
class account_journal(models.Model):
    ...
    3.4.3.7  trasaction_type - para filtrar na contabilidade
             os tipos de transação conforme abaixo
    4.1.4.2/8  self_billing - servirá para preencher os campos
               4.1.4.2 InvoiceStatus e 4.1.4.8 SelfBillingIndicator,
               nos casos de auto-facturação
    4.1.2.7    InvoiceType  classificar com um de FT, ND, NC  ou VD e
               AA alienação Activos ou DA - devol activos
    """
    _inherit = "account.journal"

    self_billing = fields.Boolean(
        string="Auto-Facturação",
        help="Assinale, se este diário se destina a registar "
             "Auto-facturação. "
             "As facturas emitidas em substituição dos fornecedores, "
             "ao abrigo de acordos de auto-facturação, são assinaladas "
             "como tal no SAFT")
    transaction_type = fields.Selection(
        ...
        selection=[('N', 'Normal'),
                   ...
                   ('J', 'Ajustamentos')],
        default="N",
        help="Categorias para classificar os movimentos contabilísticos "
             "ao exportar o SAFT")
    saft_inv_type = fields.Selection(
        string="Tipo de Documento",
        selection=[('FT', 'Factura'),
                   ('ND', 'Nota de débito'),
                   ('NC', 'Nota de Crédito'),
                   ('VD', 'Venda a Dinheiro'),
        ...
        required=False,
        default="FT",
        help="Categorias para classificar os documentos comerciais, "
             "na exportação do SAFT")


class Product(models.Model):
    _inherit = "product.product"
    _sql_constraints = [
        ('code_unique', 'unique(default_code)',
         _("Product code must be unique!"))]


tipos_saft = [
    ('C', u'Contabilidade'),   # ctb na v.1
    ('F', u'Facturação'),      # fact na v.1
    ('I', u'Integrado - Contabilidade e Facturação'),
    ('S', u'Autofacturação'),  # novo v.2
    # nao implementado:
    #('P', u'Dados parciais de facturação')
]
...
class wizard_saft(models.TransientModel):
    ...
    company_id = fields.Many2one(
        comodel_name="res.company", string="Companhia",
        required=True, default=_default_company)
    tipo = fields.Selection(
        string="Tipo ficheiro", selection=tipos_saft, default="F")
    year = fields.Many2one(
        comodel_name="account.fiscalyear", string="Exercício",
        required=True, default=_current_year)
    filter_by = fields.Selection(
        string="Filter by",
        selection=[('d', 'Dates'), ('p', 'Periods'), ],
        required=True, default='d')
    date_end = fields.Date(
        string="Data final", default=fields.Date.context_today)
    date_start = fields.Date(
        string="Data Inicial", default=fields.Date.context_today)
    period_id = fields.Many2one(
        comodel_name="account.period", string="Periodo",
    ...
    def onchange_period(self):
        if self.period_id:
            ...
            self._cr.execute(
                ...
                (self.period_id.id, self.period_id.id))
    ...
    def action_extract_saft(self, cr, uid, ids, context=None):
        ...
        tags = (
            ...
            ('CurrencyCode', 'EUR'),
            ('DateCreated', str(datetime.datetime.now().strftime('%Y-%m-%d'))),
            ('TaxEntity', 'Sede'),
        ...
        if self.this.tipo != 'C':
            root.append(self._write_source_documents(
                cr, uid, fy.date_start, fy.date_stop))
    ...
    def _get_masters(self, cr, uid, ids, context=None):
        ...
        if self.this.tipo in ('C', 'I'):
            self._cr.execute("""
                SELECT DISTINCT ac.id, ac.code, ac.name, ac.parent_id,
                       COALESCE(debito, 0.0), COALESCE(credito, 0)
                FROM account_move_line ml
                    INNER JOIN account_account  ac  ON  ac.id = ml.account_id
                    LEFT JOIN (
                        SELECT account_id, SUM( debit) AS debito,
                               SUM( credit ) AS credito
                        FROM account_move_line WHERE journal_id =
                            (SELECT open_journal FROM res_company
                             WHERE id = "+str(self.this.comp.id)+")
                            GROUP BY account_id
            ...
            for ac_id, code, name, parent, debit, credit in cr.fetchall():
                ...
                while account:
                    ...
                    try:
                        ...
                        acc_dict[account.code] = {'name': account.name,
                                                  'debit': debit,
                                                  'credit': credit}
            ...
            for code in sorted(acc_dict):
                ...
                et.SubElement(gl, 'AccountDescription').text = \
                    acc_dict[code]['name']
                et.SubElement(gl, 'OpeningDebitBalance').text = str(
                    acc_dict[code]['debit'])
                et.SubElement(gl, 'OpeningCreditBalance').text = str(
                    acc_dict[code]['credit'])
        ...
        ####   2.3 Supplier
        if self.this.tipo in ('C', 'I'):
            self._write_partners(self._cr, self._uid, self.ids, master,
    ...
    def _write_partners(self, cr, uid, ids, master, partner_role='Customer'):
        ...
        for partner in partners:
            ...
            #.1  ID
            et.SubElement(partner_element, '%sID' % partner_role).text = \
                unicode(partner.id)
            et.SubElement(partner_element, 'AccountID').text = accountId
            et.SubElement(partner_element, '%sTaxID' % partner_role).text = \
                unicode(partner.vat and partner.vat[2:] or '')
            #.4  CompanyName
            et.SubElement(partner_element, 'CompanyName').text = \
                unicode(partner.name)
            #.5 Contact   - Opcional
            #.6  BillingAddress
            billingAddress, phone_fax_mail = self.getAddress(
                cr, uid, ids, 'BillingAddress', partner.id)
            partner_element.append(billingAddress)
            # todo: .7 Ship[To|From]Address
            #8-9-10  elementos facultativos no OpenERP constam do endereço,
            #        no saft estão fora.
            for element in phone_fax_mail:
                partner_element.append(element)
            #.11 Website
            if partner.website:
                et.SubElement(partner_element, 'Website').text = \
                    unicode(partner.website)
            et.SubElement(partner_element, 'SelfBillingIndicator').text = \
                self_bill
    ...
    def _write_products(self, cr, uid, ids, master):
        ...
        for product in products:
            ...
            eproduct_description = et.SubElement(eproduct,
                                                 "ProductDescription")
    ...
    def _get_taxes(self, cr, uid, ids, context=None):
        ...
        cr.execute("""
            SELECT DISTINCT saft_tax_type, country_region, saft_tax_code,
                   name, expiration_date, type, amount
            FROM account_tax WHERE parent_id is NULL
            ORDER BY country_region, amount
            """)
        for TaxType, region, TaxCode, Description, Expiration, tipo, valor \
                in cr.fetchall():
            ...
            # opcional
                et.SubElement(taxTableEntry, 'TaxExpirationDate').text = \
                    Expiration
            if tipo == 'percent':
                et.SubElement(taxTableEntry, 'TaxPercentage').text = str(valor)
            else:
                # ToDo - amount: verify where it comes from; quick fix
    ...
    def _get_entries(self, cr, uid, ids, context=None):
        ...
        if not self.this.comp.open_journal:
            raise osv.except_osv(
                _('Error !'),
                _(u'Falta definir o diário de abertura da companhia'))

        # TODO: FIX SQL INJECTION VECTOR!
        cr.execute(
            "SELECT DISTINCT j.id, j.code, j.name, j.type, j.transaction_type "
            "FROM account_journal_period ajp "
            "    INNER JOIN account_journal j ON ajp.journal_id = j.id "
            "WHERE j.id <>" + str(self.this.comp.open_journal.id) +
            " AND ajp.period_id IN "
            "    (SELECT id FROM account_period WHERE fiscalyear_id = "
            + str(self.this.year.id) + ")")
        ...
        for j_id, j_code, j_name, j_type, transaction_type in journals:
            ...
            # TODO: FIX SQL INJECTION VECTOR!
            cr.execute("\
                SELECT m.id, m.name, p.code, m.date, \
                    COALESCE( uc.login, uw.login), m.ref,\
                ...
                WHERE m.period_id IN (\
                    SELECT id FROM account_period \
                    WHERE fiscalyear_id = " + str(self.this.year.id) + ")\
                    AND m.state = 'posted' \
                    AND m.journal_id = " + str(j_id) + " ORDER BY m.name")
            journal_transactions = cr.fetchall()
            for move_id, trans_id, period, date, user, desc, post_date \
                    in journal_transactions:
                num_entries += 1
                # 3.4.3   Transaction
                self.transactionID = unicode(
                    date + ' ' + trans_id.replace('/', ' '))
                trans_el = et.SubElement(ejournal, 'Transaction')
                et.SubElement(trans_el, 'TransactionID').text = \
                    self.transactionID
                ...
                et.SubElement(trans_el, 'DocArchivalNumber').text = \
                    unicode(trans_id.split('/')[1] )
                # todo: 3.4.3.7 TransactionType
                #- campo account_journal.transaction_type
                et.SubElement(trans_el, 'TransactionType').text = \
                    unicode(transaction_type)
                # 3.4.3.8 GLPostingDate  data no formata AAAA-MM-DD Thh:mm:ss
                et.SubElement(trans_el, 'GLPostingDate').text = \
                    date_format(post_date, 'DateTimeType')
                # 3.4.3.9|10  CustomerID|SupllierID  se diario é de vendas
                # - clientes ; se compras : fornecedores
                ...
                # TODO: FIX SQL INJECTION VECTOR!
                cr.execute(
                    "SELECT l.id, a.id, a.code, l.ref, \
                            COALESCE(l.write_date, l.create_date), \
                            l.name, l.debit, l.credit, l.partner_id \
                    FROM account_move_line l\
                        INNER JOIN account_account a ON l.account_id = a.id\
                    WHERE l.move_id = " + str(move_id) + " ORDER BY l.id")

                # Assumir que em documentos de venda e de compra
                # o parceiro é o mesmo em todas as linhas
                for line, acc_id, acc_code, source, date, \
                        ldesc, debit, credit, partner \
                        in cr.fetchall():
                    line_el = et.SubElement(trans_el, 'Line')
                    # 3.4.3.11.1  RecordID
                    #- usa o id interno da tabela account_line
                    et.SubElement(line_el, 'RecordID').text = str(line)
                    et.SubElement(line_el, 'AccountID').text = acc_code
                    et.SubElement(line_el, 'SystemEntryDate').text = \
                        date_format(date, 'DateTimeType')
                    et.SubElement(line_el, 'Description').text = ldesc
                    if debit:
                        et.SubElement(line_el, 'DebitAmount').text = str(debit)
                        total_debit += debit
                    elif credit:
                        et.SubElement(line_el, 'CreditAmount').text = \
                            str(credit)
                        total_credit += credit
                    # /Line
    ...
    def _write_invoice(self, cr, uid, invoice, eparent):
        ...
        # 4.1.4.1
        et.SubElement(eparent, u"InvoiceNo").text = unicode(
        ...
        if invoice.state == 'cancel':
            ...
            status_code = 'N'
        et.SubElement(edoc_status, u"InvoiceStatus").text = \
            unicode(status_code)
        et.SubElement(edoc_status, u"InvoiceStatusDate").text = \
            date_format(invoice.system_entry_date or invoice.write_date,
                        'DateTimeType')
        et.SubElement(edoc_status, u"SourceID").text = \
            unicode(invoice.user_id.login)
        et.SubElement(edoc_status, u"SourceBilling").text = 'P'
        # 4.1.4.3 - Hash
        et.SubElement(eparent, u"Hash").text = unicode(invoice.hash)
        # 4.1.4.4 - HasControl
        et.SubElement(eparent, u"HashControl").text = \
            unicode(invoice.hash_control)
        # 4.1.4.5 - Period          # todo: period name
        et.SubElement(eparent, u"Period").text = \
            unicode(invoice.period_id.code)
        # 4.1.4.6  InvoiceDate
        et.SubElement(eparent, u"InvoiceDate").text = \
            unicode(invoice.date_invoice)
        # 4.1.4.7 InvoiceType
        et.SubElement(eparent, u"InvoiceType").text = \
            invoice.journal_id.saft_inv_type
...
#_logger.info('A exportar Source Documents')
        ...
        for invoice in invoices:
            ...
            for line in invoice.invoice_line:
                ...
                # 4.1.4.14.15  SettlementAmount (optional)
            ...
            etax_payable.text = (invoice.amount_tax and
                unicode(invoice.amount_tax) or '0.0')
            enet_total.text = (invoice.amount_untaxed and
                unicode(invoice.amount_untaxed) or '0.0')
            ...
            if invoice.currency_id.name != 'EUR':
                ...
                # ecurrency_credit_amount = \
                #    et.SubElement(ecurrency, u"CurrencyCreditAmount")
                # ecurrency_debit_amount = \
                #    et.SubElement(ecurrency, u"CurrencyDebitAmount")
            ...
            # esettlement_discount = \
            #    et.SubElement(esettlement, u"SettlementDiscount")
            # esettlement_amount = \
            #    et.SubElement(esettlement, u"SettlementAmount")
            # esettlement_date = et.SubElement(esettlement, u"SettlementDate")
            # epayment_mechanism = \
            #    et.SubElement(esettlement, u"PaymentMechanism")
